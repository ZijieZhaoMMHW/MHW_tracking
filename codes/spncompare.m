...
for i=1:258
    ...
    mfile=matfile(['/dfs6/pub/zijiz26/nwa/mhw3/sst_anom3d' num2str(i)]);
    ...
    for j=1:length(t_here)
        ...
        for b=1:1
            ...
            if nansum(in | on)>2 && length(unique(lond(in | on)))==1
                sst_b_here=interp1(latd(in | on),mhw_here(in | on),y_full(:),'linear','extrap');
                sst_b_here=reshape(sst_b_here,res,res);
            elseif nansum(in | on)>2 && length(unique(latd(in | on)))==1
                sst_b_here=interp1(lond(in | on),mhw_here(in | on),x_full(:),'linear','extrap');
                sst_b_here=reshape(sst_b_here,res,res);
            ...
            elseif nansum(in | on)>2
                ...
                sst_b_here=nanmean(mhw_here(in | on));
            end
            %sst_b_here(~in & ~on)=0;
            sst_b(:,:,b)=sst_b_here;
            x_b(:,:,b)=x_full;
